...
class TestSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        pageCallback = re.findall(r'(?<="pageCallback":")(.*?)(?=")',
                                  str(response.text))
        ...
        script = soup.find('script', attrs={"async": True})
        print('script', script.string)

        # firstListJosn = re.findall(
        #     r'(?<=window.initialState=)(.*?)(?=</script>)',
        #     script.string)

        # firstListJosn = re.findall(pattern, str(response.text))
        # print('firstListJosn', firstListJosn)
        firstList = json.loads(
            script.string.replace('window.initialState=', ''))
    ...
    def itemsImport(self, items):
        ...
        for item in items:
            ...
            date_time_obj = datetime.datetime.fromtimestamp(t['publishTime'] /
                                                            1000)
    ...
    def getNextQuery(self, pageCallback):
        ...
        return scrapy.http.JsonRequest(
            flow_url,
            data=payload,
            callback=lambda response, payload=payload: self.nextPageParse(
                response, payload))
