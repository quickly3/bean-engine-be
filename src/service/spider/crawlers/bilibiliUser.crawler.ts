...
class BilibiliUserCrawler {
  ...
  constructor() {
    ...
    this.userDataDir = BilibiliUserCrawler.getChromeUserDataDir();
  }
  ...
  private saveApiResponseToCsv(apiResponseBody: string, pn) {
    const filename = `output/bilibili_users/${this.uid}_${pn}.csv`;
    try {
      ...
      const _list = list.map((item) => ({
        ...
        pic: item.pic,
        created: new Date(item.created * 1000).toISOString(), // 时间戳转日期时间格式
        length: item.length,
      }));
  ...
  // 示例：获取B站首页内容并尝试点击下一页
  async fetchBilibiliUser(uid): Promise<any> {
    this.uid = uid;
    if (!this.browserContext) {
      await this.launchBrowser();
    }
    const url = `https://space.bilibili.com/${this.uid}/upload/video`;
    const page = await this.browserContext!.newPage();

    // 监听请求并捕获目标API的响应
    page.on('response', async (response) => {
      const apiUrl = 'https://api.bilibili.com/x/space/wbi/arc/search';
...
export default BilibiliUserCrawler;
